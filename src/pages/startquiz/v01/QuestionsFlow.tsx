...
import { ArrowLeft, Check, Loader2, CheckCircle, ChevronDown, Car, Truck, Bike, HelpCircle, XCircle } from 'lucide-react';
...
import { storeQuizAnswer, storeFormField, getSessionData, getFinalSubmissionPayload, storeValidation, storeQuizMetadata, storeTCPAConsent, clearQuestionData, buildClickMagickParams } from '../../../core/utils/session';
import { STATES_LIST } from '../../../core/utils/stateMapping';
import { validateField, normalizePhoneNumber, validateName } from '../../../core/utils/validation';
import { config } from '../../../config/environment.config';
import { InlineOTPInput } from '../../../core/components/InlineOTPInput';
import { Footer } from '../../../core/components/Footer';
import { getStepNumber, getStepId, getStepMapping } from '../../../core/utils/quizStepMapping';
...
const QuestionsFlow: React.FC = () => {
  ...
  if (stepId) {
    const decodedStep = getStepNumber(stepId, totalSteps);
    if (decodedStep !== null) {
      initialStep = decodedStep;
      console.log(`Decoded step from URL: ${stepId} -> ${decodedStep}`);
    } else {
      console.warn(`Could not decode step ID: ${stepId}, defaulting to step 1`);
    }
  } else if (legacyStep) {
    const legacyStepNum = parseInt(legacyStep, 10);
    if (!isNaN(legacyStepNum) && legacyStepNum >= 1 && legacyStepNum <= totalSteps) {
      initialStep = legacyStepNum;
      console.log(`Legacy step parameter detected: ${legacyStep}, converting to deterministic ID`);
      const newStepId = getStepId(legacyStepNum, totalSteps);
      if (newStepId) {
        setSearchParams({ s: newStepId }, { replace: true });
      }
    } else {
      console.warn(`Invalid legacy step parameter: ${legacyStep}, defaulting to step 1`);
    }
  } else {
    console.log('No step parameter in URL, defaulting to step 1');
    initialStep = 1;
  }
  ...
  useEffect(() => {
    ...
    document.title = 'Personal Injury Hotline™ - Free Case Review';
  ...
  useEffect(() => {
    const newStepId = getStepId(currentStep, totalSteps);
    if (newStepId) {
      const currentStepId = searchParams.get('s');
      if (currentStepId !== newStepId) {
        setSearchParams({ s: newStepId }, { replace: true });
      }
    }
  }, [currentStep, totalSteps, searchParams, setSearchParams]);
  ...
  const handleButtonGroupSelect = (questionId: string, value: string) => {
    ...
    if (selectedOption?.skipTo) {
      ...
      if (skipToQuestionIndex !== -1) {
        setTimeout(() => {
          setCurrentStep(skipToQuestionIndex + 1);
        }, 300);
        return;
      }
    }
  ...
  const clearQuestionState = (stepIndex: number) => {
    ...
    if (question.type === 'button-group') {
      ...
      // Clear form-section question fields
      ...
      question.fields?.forEach(field => {
        ...
        // Reset validation states for specific fields
        if (field.id === 'zip') {
  ...
  const handleSubmit = async () => {
    ...
    setTimeout(async () => {
      ...
      try {
        ...
        const sessionData = getSessionData();
        const leadidToken = (document.querySelector('input[name="leadid_token"]') as HTMLInputElement)?.value;
        ...
        navigate(`${piQuizConfig.routes.qualified}?${queryString}`);
  ...
  // Safety check: if currentQuestion is undefined, show loading or redirect
  if (!currentQuestion) {
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-2xl mx-auto px-4 py-8 sm:py-12 mb-[50vh]">
        <div className="bg-white rounded-xl shadow-card p-6 sm:p-8 mb-6">
            ...
            <div className="space-y-3">
              {(currentQuestion.options || []).map((option) => {
                ...
                return (
                  <button
                    key={option.value}
                    onClick={() => handleButtonGroupSelect(currentQuestion.id, option.value)}
                    className={`
                      ...
                      ${isSelected ? 'border-teal bg-teal-50 ring-2 ring-teal-200' : 'border-gray-200 bg-white'}
                    `}
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {isSelected && <Check className="w-6 h-6 text-teal flex-shrink-0" />}
                      </div>
                    </div>
                  </button>
                );
          ...
          {currentQuestion.type === 'form-section' && (
            <div className="space-y-4">
              ...
              {/* Submit button for final step - positioned after all fields and before TCPA */}
              ...
              {/* TCPA Consent Text - Always at the bottom of the phone step */}
              {currentQuestion.id === 'phone' && (
                <div id="leadid_tcpa_disclosure" className="mt-4 p-4 bg-gray-50 rounded-lg border-2 border-gray-200">
                  <div
