...
import { JornayaField } from '../../../core/components/JornayaField';
import { Footer } from '../../../core/components/Footer';
import { storeQuizAnswer, storeQuizMetadata, storeFormField, getSessionData, getFinalSubmissionPayload, storeValidation, storeTCPAConsent, clearQuestionData, buildClickMagickParams } from '../../../core/utils/session';
...
const LandingPage: React.FC = () => {
  ...
  useEffect(() => {
    sessionStorage.setItem('quiz_entry_funnel', 'flow_b');
    ...
    const handleHashChange = () => {
      ...
      if (!hash || hash === 'start') {
        ...
        if (stepNumber && stepNumber >= 1 && stepNumber <= totalSteps) {
          ...
          console.warn(`Invalid hash: ${hash}, redirecting to landing`);
  ...
  useEffect(() => {
    const trackHashChange = () => {
      setTimeout(() => {
        ...
        if (stepNumber && stepNumber >= 1 && stepNumber <= totalSteps) {
          triggerJornayaHashEvent(stepNumber, hash, 'startquiz01');
        }
      }, 100);
    };
  ...
  const goToStep = (step: number) => {
    ...
    if (stepId) {
      ...
      setTimeout(() => {
        triggerJornayaHashEvent(step, stepId, 'startquiz01');
      }, 150);
    } else {
      console.warn(`Invalid step: ${step}, redirecting to landing`);
      window.location.hash = '';
      setShowHero(true);
      setCurrentStep(1);
    }
  ...
  const handleQuizStart = () => {
    if (typeof window !== 'undefined' && window.gtag) {
      window.gtag('event', 'quiz_start_startquiz_01', {
        event_category: 'engagement',
        event_label: 'Start Quiz 01 Page'
      });
    }
    setIsQuizOpen(true);
  };
  ...
  const handleButtonGroupSelect = (questionId: string, value: string) => {
    ...
    if (selectedOption?.skipTo) {
      ...
      if (skipToQuestionIndex !== -1) {
        setTimeout(() => {
          goToStep(skipToQuestionIndex + 1);
        }, 300);
        return;
      }
    }
  ...
  if (showHero) {
    return (
      <div className="min-h-screen bg-white">
        ...
        <section ref={heroRef} className="bg-gradient-to-b from-teal-50 to-white py-12 sm:py-16 lg:py-20 relative">
          <div className="absolute inset-0 bg-gradient-to-b from-white/30 via-transparent to-transparent pointer-events-none z-[1]"></div>
          <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            <div className="bg-white rounded-2xl shadow-lg border-2 border-gray-200 p-6 sm:p-8 lg:p-10 mb-8">
              <h1 className="font-heading text-2xl sm:text-3xl md:text-4xl font-bold text-navy mb-6 leading-tight text-center">
              ...
              <div className="flex flex-col gap-2 sm:flex-row sm:gap-4 justify-center items-center">
                {piContent.hero.trustElements.map((element, index) => (
                  <div key={index} className="flex items-center gap-1.5">
                    ...
                    <span className="text-xs sm:text-sm text-gray-700 font-medium whitespace-nowrap">{element}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <p className="text-xs text-gray-500 text-center mt-4 px-4 leading-relaxed">
              <span className="font-medium">Disclaimer:</span> Testimonials represent individual experiences and do not guarantee similar results. Personal Injury Hotline is not a law firm and does not provide legal advice. Case outcomes depend on specific facts and circumstances. Only licensed attorneys can evaluate your claim. Past results do not predict future outcomes.
            </p>
          </div>
        </section>
